...
# These are the patent data separators based on year
###############################################################################
...
def extract_middle_names(strings):
    # names = [(s.split(' ')[0], s.split(' ')[1]) if len(s.split(' ')) > 1 \
...
# Version 4.7 or 4.6, 4.5, 4.4, 4.3
#  * year: not needed to parse
#  * version: also not needed to parse
#  * patent_xml_string 
#  * row_dict: dictionary to populate for data extracted from XML, can replace
#  * output: print option
###############################################################################
...
###############################################################################
# process_v25_xml: processes v25 XML or SGML (note: SGML data has many missing
# closure tags which does not allow XML parsers to properly parse.)
#def process_v25_xml(year,version,patent_xml_string,index,row_dict,output=False):
###############################################################################
def process_v25_xml(year, zip_wk, patent_index, patent_xml_string, row_dict, output=False):
    ...
    try:
        ...
        # inventor_county_path = "Not found in 2.5 schema"
        # inventor_country_path = "Not found in 2.5 schema"
        ...
        # print("Issue with 2.5 {}".format(doc_number))
...
#  * year: used for output, not needed for data processing
#  * patent_doc: single APS formatted patent document 
#  * row_dict: needed to pass in existing set of processed data
...
def process_aps_txt(year, zip_wk, patent_index, patent_doc, row_dict, output=False):
    ...
    try:
        ...
        for str in single_patent_strings:
            ...
            if len(key_value_pair) == 1:
                ...
                if key_value_pair[0] == "WKU":
                    ...
                    # app_date_parts = (application_date[0:4], application_date[4:6], application_date[6:]) if application_date not in ("", None) else ("", "", "")
                elif key_value_pair[0] == "ISD":
                    publication_date = key_value_pair[1]
                    # pub_date_parts = (publication_date[0:4], publication_date[4:6], publication_date[6:]) if publication_date not in ("", None) else ("", "", "")
        ...
        # print("Issue with APS format for {}".format(doc_number))
...
def df_consolidate_csv(folder_path): # year=None):
    ...
    for key in files:
        ...
        # print("Before dropping:", frame.columns)
        frame.drop(labels="Unnamed: 0", axis='columns',inplace=True)
        # print("After dropping:", frame.columns)
...
def replace_with_codepoint(match):
    # print("This was matched on: {}".format(match.group()))
    tag = match.group()
    m = tag.lstrip('&').rstrip(';')
    try:
        #attempt = chr(name2codepoint[m])
...
def clean_anomalous_xml(year, zip_wk, patent_index, text, othercit=False):
    ...
    if year in range(2023,current_year+1) and othercit==True:
        # The following regex addresses fixing the wide variety of XML errors that appear in 2023.
        # The errors appear almost exclusively in othercit (i.e. Other Citations). An additional error appeared in Abstract. 
        # In addition to the many instances of errors, there are a wide variety of errors, making it difficult to provide a more generalized and elegant solution to accomodate all errors while
        # preserving all legal tags.  These XML errors seem to occur in 2023 from weeks 22 to 46, after
        # which the errors for the most part, completely disappear.  Note that the regex below only works up to 
        # 3925-th (of 6871) patent from 2023 week 22. 
        ...
        try:
            ...
            # if year == 2022 and zip_wk == "wk29":
            #     unclosed_tag = clean_invention_title(cleaned)
            #     cleaned = unclosed_tag
            # # 2022 wk21 <10-10> in <invention-title>
            # if year == 2022 and zip_wk == "wk21":
            #     cleaned = re.sub(r'<10-10>', "10-10", cleaned)
            # # 2022 wk19 (<5 CST) in <invention-title>
            # if year == 2022 and zip_wk == "wk19":
            #     cleaned = re.sub(r'<5 CST', "less than 5 CST", cleaned)            
            # # 2022 wk_14 <1MHz in <invention-title>
            # if year == 2022 and zip_wk == "wk14":
            #     cleaned = re.sub(r'<1 MHz', "less than 1 MHz", cleaned)
            # # 2023 wk_41 <14551> in <invention-title>
            # if year == 2023 and zip_wk == "wk41": # 
            #     cleaned = re.sub(r'(<)([0-9]+)(.*?)(\>|\<)' , "", cleaned)
            # # 2024 wk07_r1 <1mhz <invention-title>
            # if year == 2024 and zip_wk == "wk07_r1": 
            #     cleaned = re.sub(r'<1 mhz', "less than 1 mhz", cleaned)
            # # 2024 wk37 <111> in <invention-title>
            # if year == 2024 and zip_wk == "wk37":
            #     cleaned = re.sub(r'(<)([0-9]+)(.*?)(\>|\<)' , "", cleaned)
            ...
            if year == 2017 and zip_wk == "wk46":
                # cleaned = re.sub("TRUST <T>","TRUST T",cleaned)
                # cleaned = re.sub("Trust <T> Lift Corp.","Trust T Lift Corp.",cleaned)
...
def clean_patent(original_str, year, zip_wk, patent_index):
    ...
    # False: eliminate othercit and abstract tags from xml processing 
    # due to errors in input
...
# * year:  numeric year (1976 to 2024)
# * all_rows_d:  dict which will store all results for output to CSV 
# * all_files_for_year: most years (not all) can be broken down into weekly zips 
#     Boolean value - all zip files or only select range.  Exercise care- if you 
#     choose True, note that some zip files are composites which means duplicates
#     file_idx_range:  if all_files_for_year == False, this parameter is the 
#     range 0 order count with last file number need to add 1, ie 
#     zero-based indexing
# * all_patents: specify if you want all patents in the zip file.  Default True.
# * patent_idx_range:  if all_patents == False, must specify python index range.
#     Note that seequencing starts at 0 and the last index must be plus 1.
#     Useful for testing.
# * random_wait: wait time between iterations (default is not needed)
###############################################################################
def test_year(year,all_files_for_year = False, file_idx_range=(0,1),\
    ...
    for file_index in range(file_start_index, file_end_index):
        ...
        #print("weekly_patent_strings SIZE", len(''.join(weekly_patent_strings)))
...
# test_year() function which governs primary logic.  All flags and options 
# map directly to test_year arguments.  console_prt is not a command line
# flag because it is tied to multiproc.  Specifically, multiproc defaults to 
# True unless turned off by flag (eg, debugging purposes). console_prt defaults 
# to False because whenever multiproc is "on" you really do not want to
# print to console! console_prt is only turned on when the --patents flag is
# enabled. 
###############################################################################
